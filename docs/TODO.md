...
- [x] **Theme tokeny + codemod + draft/publish na produkci (2026-09-26):** hotovo — main = a650c0f, codemod aplikovaný a publikovaný u Dulpra/Svit/Dorty. Zbývá: pushnout na main opravu b83f2d6 (pořadí @import v CSS — potřeba pro dev server / editor), přidat záznam do changelogu.
- [ ] ~~**Theme tokeny + codemod (2026-09-26, `SCAFFOLD_VERSION = 5`):** po nasazení na produkci a rolloutu spustit `node scripts/codemod-inline-styles.mjs` (dry run) a pak `--apply` → u každého obchodu vznikne draft s token třídami → ve Studiu **Publish** (plný build ~1 min). Vzhled se nezmění (ověřeno na Dorty).
- [ ] **Draft / publish + Theme panel (2026-09-26, `SCAFFOLD_VERSION = 4 → 5`):** 1) PŘED deployem spustit `supabase/migration-draft-publish.sql` v Supabase SQL Editoru (bez ní vše funguje postaru — chat úpravy jdou rovnou do produkce). 2) Deploy platformy. 3) Admin → Store updates (rollout v4 — nový ThemeStyle v obchodech). 4) Otestovat ve Studiu: chat úprava živého obchodu → náhled ukazuje draft (štítek „Draft“), veřejná doména se nezmění → **Publish** → doména přepne během pár sekund; Theme panel → barvy/fonty se mění hned v náhledu → Publish (u změny jen tématu proběhne plný build ~1 min). Detail: `docs/update-log.md` → „Draft / publish + theme panel“.
