...
const GENERATE_RATE_LIMIT = 5 // per hour
const MAX_TOKENS = 8192

function makeStream(
  fn: (send: (event: object) => void) => Promise<void>
): Response {
...
export async function POST(request: Request) {
  return makeStream(async (send) => {
    ...
    if (!brief?.trim()) {
      send({ type: 'error', message: 'Brief is required.' })
      return
    }
    ...
    if ((recentCount ?? 0) >= GENERATE_RATE_LIMIT) {
      send({ type: 'error', message: `Rate limit reached — max ${GENERATE_RATE_LIMIT} generations per hour. Please wait before trying again.` })
      return
    }
    ...
    try {
      ...
      // Attempt one auto-repair: ask Claude to fix the JSON
      ...
      try {
        const repair = await anthropic.messages.create({
          ...
          max_tokens: MAX_TOKENS,
          messages: [
            { role: 'user', content: `Fix this invalid ShopManifest JSON and return ONLY the corrected JSON:\n${rawOutput}` },
          ],
        })
        const repaired = repair.content[0].type === 'text' ? repair.content[0].text : ''
        manifest = parseManifestJson(repaired)
      } catch {
    ...
    if (!projectId) {
      const { data: project, error: projError } = await supabase
        .from('projects')
        .insert({ user_id: user.id, name: projectName || manifest.brand.name, status: 'draft' })
        .select()
        .single()

      if (projError || !project) {
        send({ type: 'error', message: 'Failed to create project.' })
        return
      }
    ...
    if (versionError || !version) {
      send({ type: 'error', message: 'Failed to save manifest.' })
      return
    }
    ...
    await supabase.from('credit_ledger').insert({
      ...
      reason: 'generate',
      ref_id: version.id,
      balance_after: balance - GENERATE_COST,
    })
